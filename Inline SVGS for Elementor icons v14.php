...
class Inline_SVG_Elementor {
                        ...
                        if ( false === $safe_svg ) {
                            ...
                            if ( ! empty( $settings['custom_aria_attributes'] ) ) {
                                ...
                                if ( json_last_error() !== JSON_ERROR_NONE ) {
                                    $this->log_error( 'Invalid JSON for ARIA attributes in widget settings.' );
                                    $custom_aria = [];
                                }
                                ...
                                foreach ( $custom_aria as $attr => $value ) {
                                    ...
                                    if ( in_array( $attr, $allowed_aria_attributes, true ) ) {
                                        ...
                                        $this->log_error( "Unsupported ARIA attribute: {$attr}." );
                                    }
                                }
                            }
    ...
    public function clear_cache() {
        ...
        // Update cache version to invalidate old cache keys.
        $this->cache_version++;
        update_option( 'inline_svg_elementor_cache_version', $this->cache_version );
    }
